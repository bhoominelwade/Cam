...
- S2 done on branch `s2-engine-v1` (off s1): portrait rule (upper-thirds placement 60% + face size 40%, celebrate ≥85), computeGuidance dispatch, 19 engine tests incl. 2000-case fuzz; app wiring via `file:` dep + metro watchFolders, nudge hysteresis 300ms, ScoreBadge/target-zone/chevron overlays.
- Blocked on astlin: Apple Developer Program enrollment ($99) not completed — device:create failed with "no team associated". After enrollment: `npx eas-cli device:create` then `npx eas-cli build --profile development --platform ios` in `app/` on branch `s2-engine-v1` (build carries S0+S1+S2); run e2e-checklists s0→s2 on the iPhone, tune rotation/mirror constants in `app/src/detection/constants.ts`, merge slices in order once checks pass. PowerShell gotcha: don't pass commit messages with embedded double quotes via here-string — use `git commit -F <file>`.
